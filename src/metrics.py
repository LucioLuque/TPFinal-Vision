...
def calculate_psnr(image_true, image_test, data_range=255):
...
def calculate_ssim(image_true, image_test, data_range=255):
    np_image_true = np.array(image_true)
    np_image_test = np.array(image_test)
    return ssim(np_image_true, np_image_test, data_range=data_range)

def calculate_average_metrics(images_true, images_test, data_range=255):
    psnr_values = []
    ssim_values = []
    ...
    for image_true, image_test in zip(images_true, images_test):
        ...
        ssim_values.append(calculate_ssim(image_true, image_test, data_range=data_range))
